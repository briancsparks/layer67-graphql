...
const project                 = ARGV.project  || ARGV.project_id || 'starwars';
...
app.use(`/${project}/graphql`, bodyParser.json(), graphqlExpress({ schema }));

// GraphiQL, a visual editor for queries
app.use(`/${project}/graphiql`, graphiqlExpress({ endpointURL: `/${project}/graphql` }));

// Start the server
app.listen(port, () => {
  console.log(`Go to http://localhost:3000/${project}/graphiql to run queries!`);

  tell();
  function tell() {
    setTimeout(tell, 15000);

    layer67.tellStackService(`/${project}/graphql`, `http://${ip}:${port}`, 30000, stack, function(err) {
      layer67.tellStackService(`/${project}`, `http://${ip}:${port}`, 30000, stack, function(err) {
      });
    });
  }
});
